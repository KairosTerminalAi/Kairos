...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Kairos PKCE ({_HERMES_OAUTH_FILE})",
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Kairos inside WSL2 to use the dashboard's /chat "
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Kairos Teal",         "description": "Classic dark teal — the canonical Kairos look"},
    {"name": "default-large", "label": "Kairos Teal (Large)", "description": "Kairos Teal with bigger fonts and roomier spacing"},
...
def start_server(
    ...
    *,
    ...
    print(f"  Kairos Web UI → http://{host}:{port}")
