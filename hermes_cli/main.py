...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Kairos?")
        print()
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Kairos only falls back to a lightweight backend (OpenRouter,")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Kairos will probe your")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Kairos turns to `copilot --acp`.")
    print("  Kairos currently starts its own ACP subprocess for each request.")
    print("  Kairos uses your selected model as a hint for the Copilot ACP session.")
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Kairos typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Kairos, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Kairos will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_import(args):
    """Restore a Kairos backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"KAIROS-AGENT v{__version__} ({__release_date__})")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Kairos behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Kairos couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Kairos couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Kairos behaves unexpectedly.")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Kairos repository.")
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Kairos Agent")
        return
    ...
    try:
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Kairos gateway unit(s) detected:")
...
def main():
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        description=(
            "Stop, disable, and remove legacy Kairos gateway unit files "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Kairos.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Kairos")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Kairos CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Kairos with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Kairos Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Kairos Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Kairos setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Kairos Agent. Use 'kairos debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Kairos home directory to a zip file",
        description="Create a zip archive of your entire Kairos configuration, "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Kairos Agent configuration",
    )
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Kairos as an MCP server",
        description=(
            "Manage MCP server connections and run Kairos as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hermes mcp add' to connect to a new server, or\n"
            "'kairos mcp serve' to expose Kairos conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Kairos as an MCP server (expose conversations to other agents)",
    )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Kairos Agent to the latest version",
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Kairos Agent as an ACP (Agent Client Protocol) server",
        description="Start Kairos Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Kairos Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Kairos instances",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Kairos profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Kairos Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Kairos log files",
