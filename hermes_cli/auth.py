...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'kairos model' to choose a "
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Kairos:")
    print(authorize_url)
    print()
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Kairos is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Kairos is not logged into Nous Portal.",
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Kairos auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Kairos will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Kairos will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(Kairos creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Kairos login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Kairos login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Kairos will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `kairos model` or configure an API key to use Kairos.")
        else:
