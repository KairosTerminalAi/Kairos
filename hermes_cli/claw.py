...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Kairos may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "Kairos gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def _cmd_migrate(args):
    ...
    if not no_backup:
        try:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Kairos home."
            )
